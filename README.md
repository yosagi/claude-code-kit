...
#### 2.3 プロジェクトタイトルの確認（任意）

セットアップ完了時に `reports/project_title.txt` が生成されます。`claude-code` ラッパー（osc-tap 経由）で起動すると、この内容が1行表示されます。

```
プロジェクト名 / 人格名 : 概要
```

概要はセットアップ時に仮で生成されるため、必要に応じて手動で修正してください。

#### 2.4 セッションログ出力の有効化（任意）
