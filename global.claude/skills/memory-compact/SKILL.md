---
name: memory-compact
description: 記憶ファイル（日記、work_history）の古いエントリを要約して圧縮する
context: fork
user_invocable: true
---
...
記憶ファイルのエントリが上限を超えたとき、古いエントリを要約して圧縮する。
archive ファイルには session_end hook で自動追記済みのため、archive への書き込みは不要。
...
1. 元ファイルから対象エントリを削除
2. 要約を元ファイルの「過去の要約」セクションに追記（新しいものが上。「過去の要約」セクションが存在しなければファイル末尾に作成）
...
- アーカイブ: 変更なし（session_end hook で既に追記済み）
